...
export function deterministicTrimSystemPrompt(
  compiled: CompiledPersona
): TrimResult {
  if (!compiled?.systemPrompt) {
    throw new ExporterError(
      "INVALID_INPUT",
      "Compiled persona is required to export."
    );
  }
...
function materializePinnedLines(compiled: CompiledPersona): string[] {
  const context: Record<string, string> = {
    tone: compiled.resolvedConfigSnapshot.tone,
    pacing: compiled.resolvedConfigSnapshot.pacing,
    tone_description:
      compiled.derived.toneDescription ?? compiled.resolvedConfigSnapshot.tone,
    pacing_description:
      compiled.derived.pacingDescription ??
      compiled.resolvedConfigSnapshot.pacing,
  };

  return CANONICAL_PINNED_LINES.map((line) =>
    line.replace(/{{\s*([^}]+)\s*}}/g, (_, key: string) => context[key] ?? "")
  );
}
