...
export function validatePersonaConfig(config: PersonaConfig): void {
  if (!isSupportedSchemaVersion(config.schemaVersion)) {
    throw new CompileError(
      ...
      `Unsupported schemaVersion "${config.schemaVersion}"`
    );
  }
  ...
  if (
    ...
    config.jobs.some(
      (job) => typeof job !== "string" || job.trim().length === 0
    )
  ) {
  ...
  const optionalStringArrays: Array<[unknown, string]> = [
  ...
  if (
    config.pressureStyle !== undefined &&
