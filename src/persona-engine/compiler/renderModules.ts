...
function renderModule(
  ...
  switch (id) {
    ...
    case "pressure_style": {
      const signatureLines =
        resolved.signaturePhrases.length > 0
    ...
    case "tone_pacing": {
      const pinned = CANONICAL_PINNED_LINES.map((line) =>
        interpolatePinned(line, resolved, derived)
      );
      const lines = [
    ...
    case "info_style_library": {
      ...
      const lines = resolved.infoStyles.map(
        (style) => `- info_style: ${style}`
      );
      return joinLines(lines);
    }
  ...
  throw new CompileError(
    "RENDER_FAILURE",
    `Unhandled module id ${String(exhaustiveCheck)}`
  );
}
...
export function renderModules(
  resolved: ResolvedPersonaConfig,
  derived: DerivedPersonaFields,
  options?: { includeTestChat?: boolean }
): CompiledModule[] {
  const shouldIncludeTestChat = options?.includeTestChat ?? false;

  const modules = MODULE_REGISTRY.filter(
    (module) => shouldIncludeTestChat || module.kind !== "test_chat"
  ).sort((a, b) => a.order - b.order);

  return modules
    .map<CompiledModule | null>((definition) => {
      ...
      if (isEmpty && !definition.droppable) {
        throw new CompileError(
          "RENDER_FAILURE",
          `Module "${definition.id}" rendered empty`
        );
      }
    ...
    .filter((module): module is CompiledModule => module !== null);
}
