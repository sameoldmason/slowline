...
function isValidIntent(intent) {
    return typeof intent === "string" && KNOWN_INTENTS.includes(intent);
}
function resolveIntent(runtime) {
    if (runtime.intent !== undefined && runtime.intent !== null) {
        if (!isValidIntent(runtime.intent)) {
            throw new RouterError("INVALID_INTENT", `Unsupported intent "${String(runtime.intent)}"`);
        }
...
function isPermissionAskingEnabled(config) {
    const tone = config.tone.trim().toLowerCase();
    const pacing = config.pacing.trim().toLowerCase();
    return tone === "soft" || tone === "warm" || pacing === "slow";
}
function detectFlaggedBoundaries(boundaries, message) {
    const normalizedMessage = message.toLowerCase();
    return boundaries.filter((boundary) => normalizedMessage.includes(boundary.toLowerCase()));
}
...
function buildRoutedPlan(intent, context, permissionRequiredForQuestions, flaggedBoundaries) {
    const { resolvedConfigSnapshot } = context.persona;
    const applyPressure = intent !== "vent_reflection";
    const allowAdvice = intent !== "vent_reflection" ||
...
function assertRuntimeInput(runtime) {
    if (!runtime ||
        typeof runtime.message !== "string" ||
        runtime.message.trim().length === 0) {
        throw new RouterError("INVALID_INPUT", "runtime.message is required for routing");
    }
}
export function routeResponse(context) {
    ...
    const permissionRequiredForQuestions = isPermissionAskingEnabled(context.persona.resolvedConfigSnapshot);
    const flaggedBoundaries = detectFlaggedBoundaries(context.persona.resolvedConfigSnapshot.boundaries, context.runtime.message);
    const plan = intent === null
        ? buildClarifyPlan()
        : buildRoutedPlan(intent, context, permissionRequiredForQuestions, flaggedBoundaries);
